...
app.post('/api/codes/generate-special', authMiddleware, adminMiddleware, async (req, res) => {
  ...
  const typeName = { 1: 'مفردة', 2: 'ثنائية', 3: 'ثلاثية', 4: 'رباعية', 5: 'خماسية', 6: 'سداسية', 7: 'سباعية', 8: 'ثمانية', 9: 'تساعية' }[parseInt(length)] || '';
...
app.post('/api/watch/upload', authMiddleware, async (req, res) => {
  try {
    ...
    // نستلم الملف فوراً (بدون انتظار التحويل) حتى لا تنقطع الاستجابة
    ...
    res.json({ ok: true, url: fileUrl, pending: true });
    // التحويل يتم في الخلفية ثم يبدأ البث عندما يكون الملف جاهزاً
    const meName = meUser?.name || 'المؤسس';
    (async () => {
      try {
        let finalBuf = buf;
        ...
        const f = globalThis.watchFiles[sessionId];
        if (f) {
          f.buffer = finalBuf;
          f.type = finalType;
          f.converting = false;
      ...
      // بدء جلسة المشاهدة — يبدأ متوقفاً (زر تشغيل واضح)
      globalThis.watchSessions[sessionId] = {
        url: fileUrl, playing: false, time: 0, byName: meName, ts: Date.now()
      };
      io.to(`session_${sessionId}`).emit('watch_started', {
        url: fileUrl, playing: false, time: 0, byName: meName
      });
...
app.post('/api/admin/gift-items/add', authMiddleware, adminMiddleware, async (req, res) => {
  const { name, emoji, coins, gift_image, price, start_date, end_date } = req.body;
  if (!name) return res.status(400).json({ error: 'اسم الهدية مطلوب' });
  const gift = await db.addGiftItem(name, emoji, coins, gift_image, price, start_date, end_date);
  res.json({ message: '✅ تمت إضافة الهدية', gift });
});
app.post('/api/admin/gift-items/update', authMiddleware, adminMiddleware, async (req, res) => {
  const { id, name, emoji, coins, price, status, gift_image, start_date, end_date } = req.body;
  const gift = await db.updateGiftItem(id, { name, emoji, coins, price, status, gift_image, start_date, end_date });
...
app.post('/api/livekit/token', authMiddleware, asyncHandler(async (req, res) => {
  ...
  at.addGrant({
    roomJoin: true,
    room: room,
    canPublish: role === 'host',          // المضيف يبث
    canPublishData: true,
    canSubscribe: true,
    canPublishSources: role === 'host' ? ['camera','microphone','screen_share'] : [],
  });
  const token = await at.toJwt();
  res.json({ token, url: LIVEKIT_URL, identity, role });
}));
